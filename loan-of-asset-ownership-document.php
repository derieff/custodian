...
if(isset($_GET["act"])) {
	...
	if($act=='add') {
		...
		<tr>
			<td width='30'>Nama</td>
			<td width='70%'>
				<input name='txtTHLOAOD_UserID' type='hidden' value='$_SESSION[User_ID]'/>
				$field[FullName]
			</td>
		</tr>
		<tr>
	...
	elseif($act=='adddetail')	{
		...
		<tr>
			...
			if( $field['THLOAOD_DocumentType'] == "ORIGINAL" ){
				$ActionContent .="Asli";
			}elseif( $field['THLOAOD_DocumentType'] == "HARDCOPY" or $field['THLOAOD_DocumentType'] == "SOFTCOPY" ){
				$ActionContent .= ucfirst(strtolower($field['THLOLAD_DocumentType']));
			}else{
				if( $field['THLOAOD_LoanCategoryID'] != '3') $ActionContent .= "Asli";
				else $ActionContent .= "";
			}
			$ActionContent .="</td>
		</tr>
		<tr>
			<td>Dokumen dengan Cap/Watermark</td>
			<td colspan='2'><input type='hidden' name='optTHLOLAD_LoanCategoryID' value='$field[THLOAOD_DocumentWithWatermarkOrNot]'>";
...
elseif(isset($_POST['canceldetail'])) {
	$cd_query="UPDATE M_CodeTransaction ct, TH_LoanOfAssetOwnershipDocument thloaod
			   SET ct.CT_Delete_UserID='$_SESSION[User_ID]',ct.CT_Delete_Time=sysdate(),
			       ...
			       thloaod.THLOAOD_Delete_UserID='$_SESSION[User_ID]',thloaod.THLOAOD_Delete_Time=sysdate(),
			       thloaod.THLOAOD_Update_UserID='$_SESSION[User_ID]',thloaod.THLOAOD_Update_Time=sysdate()
