...
if(isset($_GET["act"]))
{
	...
	if($act=='add') {
		...
		<tr>
			<td width='30'>Nama</td>
			<td width='70%'>
				<input id='txtTHROOLD_UserID' type='hidden' value='$_SESSION[User_ID]'/>
				$field[FullName]
			</td>
		</tr>
		<tr>
	...
	for ($i=0 ; $i<$sum ; $i++) {
		...
		$sql1= "INSERT INTO TD_ReleaseOfOtherLegalDocuments
				VALUES (NULL,NULL,'$_POST[txtTDROOLD_THROOLD_ID]', '$TDLOOLD_ID[$i]','$txtTDROOLD_Information[$i]',
						'$TDROOLD_LeadTime',NULL,'$_SESSION[User_ID]', sysdate(),'$_SESSION[User_ID]',
						sysdate(),NULL,NULL)";
		$sql2= "UPDATE TD_LoanOfOtherLegalDocuments
				SET TDLOOLD_Response='1', TDLOOLD_Update_UserID='$_SESSION[User_ID]',TDLOOLD_Update_Time=sysdate()
