...
IF ($txtTHLOLD_DocumentGroupID<>"3"){
	...
	$query="SELECT DISTINCT dt.DocumentType_Name, c.Company_Name, dg.DocumentGroup_Name, dc.DocumentCategory_Name, 
							dl.DL_RegTime, dl.DL_DocCode, dl.DL_NoDoc, dl.DL_Instance, dl.DL_PubDate, dl.DL_ExpDate, 
	...
	if ($numRow==0) {
		...
		if($_POST) {
			...
			$query =   "SELECT dt.DocumentType_Name, dl.DL_RegTime, dl.DL_DocCode, dl.DL_NoDoc, dl.DL_Instance, dl.DL_PubDate, dl.DL_ExpDate, 
							   di1.DocumentInformation1_Name, di2.DocumentInformation2_Name, dl.DL_Information3
		...
		while ($arr=mysql_fetch_array($sql)){
			...
			<tr>
				...
				<td align='center'><?= $arr['DL_Information3'] ?></td>
