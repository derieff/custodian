...
<?PHP
$DOL_DocCode=$_GET["cBarcodePrint"];
$jumlah=count($DOL_DocCode);
...
for($i=1;$i<=$jumlah;$i++){
	$query="SELECT dol.DOL_DocCode,
				   c.Company_Name,
                   m_dc.DocumentCategory_Name kategori_dokumen,
				   dol.DOL_NamaDokumen, dol.DOL_NoDokumen,
				   dol.DOL_Location
		  	FROM M_DocumentsOtherLegal dol, M_Company c, db_master.M_DocumentCategory m_dc
			WHERE dol.DOL_DocCode='".$DOL_DocCode[$i-1]."'
			AND dol.DOL_CompanyID=c.Company_ID
            AND m_dc.DocumentCategory_ID=DOL_CategoryDocID";
	$sql = mysql_query($query);
	$arr = mysql_fetch_array($sql);

    $grup_document = "Dokumen Lainnya (Legal)";
    $stringData = "";

	if ($i%2==0){
		echo"<td></td>";
		$stringData .= "^BY2,2.5,60^FT450,173^BAN,N,,N,N\n";
		$stringData .= "^FD".$arr['DOL_DocCode']."^FS\n";
		$stringData .= "^FT450,23^A0N,17,16^FH\^FD".substr("$arr[Company_Name]",0,25)."^FS\n";
		$stringData .= "^FT820,23,1^A0N,17,16^FH\^FD".$arr['DOL_Location']."^FS\n";
		$stringData .= "^FT450,43^A0N,17,16^FH\^FD".$grup_document."^FS\n";
		$stringData .= "^FT450,63^A0N,17,16^FH\^FD".$arr['kategori_dokumen']."^FS\n";
		$stringData .= "^FT450,83^A0N,17,16^FH\^FD".$arr['DOL_NoDokumen']."^FS\n";
		$stringData .= "^FT450,103^A0N,17,16^FH\^FD".$arr['DOL_NamaDokumen']."^FS\n";
		...
		$stringData .= "^FT400,23,1^A0N,17,16^FH\^FD".$arr['DOL_Location']."^FS\n";
		$stringData .= "^FT25,43^A0N,17,16^FH\^FD".$grup_document."^FS\n";
		$stringData .= "^FT25,63^A0N,17,16^FH\^FD".$arr['kategori_dokumen']."^FS\n";
		$stringData .= "^FT25,83^A0N,17,16^FH\^FD".$arr['DOL_NoDokumen']."^FS\n";
		$stringData .= "^FT25,103^A0N,17,16^FH\^FD".$arr['DOL_NamaDokumen']."^FS\n";
	}
	echo"
	...
	<tr>
		...
		<td align='right'>$arr[DOL_Location]</td>
	</tr>
	<tr>
		<td colspan='2'>$grup_document</td>
	</tr>
	<tr>
		<td colspan='2'>$arr[kategori_dokumen]</td>
	</tr>
	<tr>
		<td colspan='2'>$arr[DOL_NoDokumen]</td>
	</tr>
    <tr>
		<td colspan='2'>$arr[DOL_NamaDokumen]</td>
	</tr>
	<tr>
		<td colspan='2'>&nbsp;</td>
	</tr>
	<tr>
		<td colspan='2' align='center'>
			<img src='barcode.php?text=$arr[DOL_DocCode]'><br>
			$arr[DOL_DocCode]
		</td>
	</tr>
	</table>
