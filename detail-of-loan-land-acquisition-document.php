...
<tr>
	<td>Tipe Dokumen</td>
	<td colspan='2'><input type='hidden' name='optTHLOLAD_DocumentType' value='$arr[THLOLAD_DocumentType]'>";
	if( $arr['THLOLAD_DocumentType'] == "ORIGINAL" ){
		$MainContent .="Asli";
	}elseif( $arr['THLOLAD_DocumentType'] == "HARDCOPY" or $arr['THLOLAD_DocumentType'] == "SOFTCOPY" ){
		$MainContent .= ucfirst(strtolower($arr['THLOLAD_DocumentType']));
	}else{
		if( $arr['THLOLAD_LoanCategoryID'] != '3') $MainContent .= "Asli";
		else $MainContent .= "";
	}
	$MainContent .="</td>
</tr>
<tr>
	...
	<td colspan='2'><input type='hidden' name='optTHLOLAD_DocumentWithWatermarkOrNot' value='$arr[THLOLAD_DocumentWithWatermarkOrNot]'>";
		if( $arr['THLOLAD_DocumentWithWatermarkOrNot'] == "1" ){
			$MainContent .="Iya";
		}elseif( $arr['THLOLAD_DocumentWithWatermarkOrNot'] == "2" ){
			$MainContent .="Tidak";
		}else{
			$MainContent .= "-";
		}
