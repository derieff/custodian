	...
	if ($num==1) {
		...
		if ($AppDate==NULL) {
			...
			if ($A_Status=='3') {
				...
				if ($step <> $jStep) { //Bukan Approval Final
					$nStep=$step+1;
					//echo $nStep; echo '<br />';
		...
		<tr>
			<td class='footer'>Powered By Custodian System </td>
					...
					if ($sql = mysql_query($query)) {
						...
						$d_query="SELECT *
								  FROM TD_LoanOfAssetOwnershipDocument
								  WHERE TDLOAOD_THLOAOD_ID='$h_arr[THLOAOD_ID]'
								  AND TDLOAOD_Delete_Time IS NULL";
						...
						while($d_arr=mysql_fetch_array($d_sql)){
							...
							$query1="UPDATE M_DocumentAssetOwnership
									 SET DAO_Status ='$docStatus',DAO_Update_Time=sysdate(),DAO_Update_UserID='$A_ApproverID'
							...
							$query3="UPDATE TD_LoanOfAssetOwnershipDocument
									 SET TDLOAOD_Code ='$CT_Code',TDLOAOD_Update_Time=sysdate(),
										 TDLOAOD_Update_UserID='$A_ApproverID'
									 WHERE TDLOAOD_THLOAOD_ID='$h_arr[THLOAOD_ID]'
									 AND TDLOAOD_DocCode='$d_arr[TDLOAOD_DocCode]'";
	...
	if ($act=='reject'){
		...
		<tr>
			<td><input type='hidden' name='A_ID' value='$A_ID'>
				...
				<textarea name='txtTHLOAOD_Reason' id='txtTHLOAOD_Reason' rows='3'>$arr[THLOAOD_Reason]</textarea>
...
if(isset($_POST['reject'])) {
	...
	if (str_replace(" ", "", $_POST['txtTHLOAOD_Reason'])==NULL){
		...
		if ($num==1) {
			...
			if ($AppDate==NULL) {
				...
				$query1="UPDATE TH_LoanOfAssetOwnershipDocument
						  SET THLOAOD_Status='reject', THLOAOD_Reason='$THLOAOD_Reason',
							  THLOAOD_Update_Time=sysdate(), THLOAOD_Update_UserID='$A_ApproverID'
						  WHERE THLOAOD_LoanCode='$A_TransactionCode'";
				...
				while($d_arr=mysql_fetch_array($d_sql)){
					...
					$query1="UPDATE M_DocumentAssetOwnership
							 SET DAO_Status ='1',DAO_Update_Time=sysdate(),DAO_Update_UserID='$A_ApproverID'
							 WHERE DAO_DocCode='$d_arr[TDLOAOD_DocCode]'";

					$mysqli->query($query1);
				}
				mail_notif_loan_doc($A_TransactionCode, $h_arr['THLOAOD_UserID'], 4 );
				$e_query="SELECT *
						  FROM M_Approval
						  WHERE A_TransactionCode='$A_TransactionCode'
						  AND A_Step<'$step' ";
