...
function mail_loan_doc($loanCode,$reminder=0){
	...
	$e_query ="	SELECT 	User_ID,User_FullName,User_Email,DocumentGroup_Name,A_TransactionCode,
					ARC_AID,ARC_RandomCode,THLOLAD_Information,THLOLAD_LoanDate,THLOLAD_LoanCategoryID,
					THLOLAD_DocumentType, THLOLAD_DocumentWithWatermarkOrNot, # Arief F - 07092018
				...
				LEFT JOIN M_LoanCategory
					ON THLOLAD_LoanCategoryID = LoanCategory_ID
	...
	//$mail->AddAttachment("images/icon_addrow.png", "icon_addrow.png");  // optional name
		$ed_query="	SELECT DISTINCT TDLOLAD_ID,THLOLAD_LoanCategoryID, Company_Name,DLA_Phase,DLA_Period,DLA_Village,DLA_Block,DLA_Owner,
		...
		if($row->THLOLAD_DocumentType == "ORIGINAL" ){ //Arief F - 07092018
			...
			if( $row->THLOLAD_LoanCategoryID == '1' or $row->THLOLAD_LoanCategoryID == '2' ) $tipe_dokumen = "Asli";
			elseif( $row->THLOLAD_LoanCategoryID == '3') $tipe_dokumen = "Hardcopy";
			elseif( $row->THLOLAD_LoanCategoryID == '4') $tipe_dokumen = "Softcopy";
		...
		// $asli = ($row->THLOLAD_LoanCategoryID != '3') ? ' Asli ' : '';
		$keteranganPermintaan = ""; //Arief F - 07092018
		if( $row->THLOLAD_Information != null or $row->THLOLAD_Information != "" ){ //Arief F - 07092018
			$keteranganPermintaan = "(tujuan permintaan dokumen adalah ".$row->THLOLAD_Information.")"; //Arief F - 07092018
		} //Arief F - 07092018
		$bodyHeader = '
...
function mail_notif_loan_doc($loanCode, $User_ID, $status, $attr){
	...
	//$mail->AddAttachment("images/icon_addrow.png", "icon_addrow.png");  // optional name
		...
		$ed_query = "SELECT DISTINCT TDLOLAD_ID,Company_Name,THLOLAD_LoanCategoryID,LoanCategory_Name,
			 			DLA_Phase,DLA_Period, DLA_Village,DLA_Block,DLA_Owner, User_FullName, DLA_AreaStatement,
			 			DLA_PlantTotalPrice, DLA_GrandTotal, DLA_DocDate, THLOLAD_Reason,
						THLOLAD_DocumentType, THLOLAD_DocumentWithWatermarkOrNot, # Arief F - 07092018
			 			THLOLAD_UserID,Company_ID,THLOLAD_Information
					FROM TH_LoanOfLandAcquisitionDocument
					LEFT JOIN TD_LoanOfLandAcquisitionDocument
						ON TDLOLAD_THLOLAD_ID=THLOLAD_ID
					LEFT JOIN M_DocumentLandAcquisition
					...
					LEFT JOIN M_LoanCategory
						ON THLOLAD_LoanCategoryID = LoanCategory_ID
					LEFT JOIN M_CodeDocument
						ON DLA_Code = CD_Code
					LEFT JOIN M_DocumentGroup
						ON CD_GroupDocCode = DocumentGroup_Code
					LEFT JOIN M_Company
						ON Company_ID=THLOLAD_CompanyID
					LEFT JOIN M_User
						ON THLOLAD_UserID=User_ID
		...
		$body .= '
			...
			if($ed_arr->THLOLAD_DocumentType == "ORIGINAL" ){ //Arief F - 07092018
				$tipe_dokumen = "Asli"; //Arief F - 07092018
			}elseif($ed_arr->THLOLAD_DocumentType == "HARDCOPY" or $ed_arr->THLOLAD_DocumentType == "SOFTCOPY"){ //Arief F - 07092018
				$tipe_dokumen = ucfirst(strtolower($ed_arr->THLOLAD_DocumentType)); //Arief F - 07092018
			}else{ //Arief F - 07092018
				if( $ed_arr->THLOLAD_LoanCategoryID == '1' or $ed_arr->THLOLAD_LoanCategoryID == '2' ) $tipe_dokumen = "Asli";
				elseif( $ed_arr->THLOLAD_LoanCategoryID == '3') $tipe_dokumen = "Hardcopy";
				elseif( $ed_arr->THLOLAD_LoanCategoryID == '4') $tipe_dokumen = "Softcopy";
				else $tipe_dokumen = ""; //Arief F - 07092018
			} //Arief F - 07092018
			$dengan_cap = "";
			if( $tipe_dokumen == "Hardcopy" || $tipe_dokumen == "Softcopy" ){
				if( $ed_arr->THLOLAD_DocumentWithWatermarkOrNot == "1" ){ //Arief F - 07092018
					$dengan_cap = " dengan ".$cap_atau_watermark; //Arief F - 07092018
				}elseif( $ed_arr->THLOLAD_DocumentWithWatermarkOrNot == "2" ){ //Arief F - 07092018
					$dengan_cap = " tanpa ".$cap_atau_watermark; //Arief F - 07092018
				}
			}
			// $asli = ($row->THLOLAD_LoanCategoryID != '3') ? ' Asli ' : '';
			$keteranganPermintaan = ""; //Arief F - 07092018
			if( $ed_arr->THLOLAD_Information != null or $ed_arr->THLOLAD_Information != "" ){ //Arief F - 07092018
...
<tr>
	...
	<div style="margin-bottom: 15px">
	<p><span style="margin-bottom: 15px; font-size: 13px; font-family: \'lucida grande\',tahoma,verdana,arial,sans-serif;">';
	// $bodyHeader .= 'Bersama ini disampaikan bahwa permintaan dokumen  <b>('.$loanName.$asli.')</b>';
