...
function mail_loan_doc($loanCode,$reminder=0){
	...
	//$mail->AddAttachment("images/icon_addrow.png", "icon_addrow.png");  // optional name
		...
		$ed_query="	SELECT DISTINCT	Company_Name,
					...
					LEFT JOIN M_Company
						ON Company_ID=THLOAOD_CompanyID
...
function mail_notif_loan_doc($loanCode, $User_ID, $status, $attr){
	...
	//$mail->AddAttachment("images/icon_addrow.png", "icon_addrow.png");  // optional name
		...
		// $ed_query="	SELECT DISTINCT	Company_Name,THLOAOD_LoanCategoryID, DocumentCategory_Name,DocumentType_Name,
		// 							DAO_NoDoc,DAO_RegTime,THLOAOD_Reason,THLOAOD_UserID,User_FullName,
		// 							THLOAOD_Information,Company_ID, LoanCategory_Name,DocumentGroup_Name
		// 			FROM TH_LoanOfAssetOwnershipDocument
		// 			LEFT JOIN TD_LoanOfAssetOwnershipDocument
		// 				ON TDLOAOD_THLOAOD_ID=THLOAOD_ID
		// 			LEFT JOIN M_LoanCategory
		...
		// 				ON THLOAOD_DocumentGroupID=DocumentGroup_ID
		// 			LEFT JOIN M_Company
		// 				ON Company_ID=THLOAOD_CompanyID
		// 			LEFT JOIN M_DocumentAssetOwnership
		// 				ON TDLOAOD_DocCode=DAO_DocCode
		// 			LEFT JOIN M_DocumentCategory
		// 				ON DocumentCategory_ID=TDLOAOD_DocumentCategoryID
		// 			LEFT JOIN M_DocumentType
		// 				ON DocumentType_ID=DAO_TypeDocID
		// 			LEFT JOIN M_User
		// 				ON THLOAOD_UserID=User_ID
		// 			WHERE THLOAOD_LoanCode='$loanCode'
		// 			AND THLOAOD_Delete_Time IS NULL";

		$ed_query="	SELECT DISTINCT	Company_Name,THLOAOD_LoanCategoryID,
						THLOAOD_DocumentType, THLOAOD_DocumentWithWatermarkOrNot,
						DAO_RegTime,THLOAOD_Reason,THLOAOD_UserID,User_FullName,
