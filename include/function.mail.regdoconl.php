...
<tr>
	...
	<p>
		...
		$bodyFooter .= '
				...
				if ($custodian==1){
			$bodyFooter .= '
				<span style="border: 1px solid green;padding: 5px;margin-bottom: 15px; font-size: 13px;font-family: \'lucida grande\',tahoma,verdana,arial,sans-serif;"><a target="_BLANK" href="http://'.$_SERVER['HTTP_HOST'].'/custodian/detail-of-registration-document.php?act='.$decrp->encrypt('approve').'&ati='.$decrp->encrypt($row->ARC_AID).'&rdm='.$decrp->encrypt($row->ARC_RandomCode).'&id='.$decrp->encrypt($row->THROONLD_ID).'">Revisi</a></span><br />';
				}
				else {
			$bodyFooter .= '
				<br>';
