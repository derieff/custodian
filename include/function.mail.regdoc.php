...
function mail_registration_doc($regCode,$reminder=0){
	...
	//$mail->AddAttachment("images/icon_addrow.png", "icon_addrow.png");  // optional name
		...
		$ed_query="	SELECT DISTINCT	Company_Name,DocumentCategory_Name,DocumentType_Name,TDROLD_DocumentNo,TDROLD_DatePublication,User_FullName,
					...
					LEFT JOIN M_Company
						ON Company_ID=THROLD_CompanyID
			...
			$body .= '
						<TR  style=" font-size: 12px; font-family: \'lucida grande\',tahoma,verdana,arial,sans-serif;">
							...
							<TD>'.$ed_arr->Company_Name.'<br />
								...
								No. Dokumen : '.$ed_arr->TDROLD_DocumentNo.'<br />
								Tgl. Terbit : '.date('d/m/Y H:i:s', strtotime($ed_arr->TDROLD_DatePublication)).'
							</TD>
						</TR>';
			$edNum=$edNum+1;
...
function mail_notif_registration_doc($regCode, $User_ID, $status, $attr){
	...
	//$mail->AddAttachment("images/icon_addrow.png", "icon_addrow.png");  // optional name
		...
		$ed_query="	SELECT DISTINCT	Company_Name,DocumentCategory_Name,DocumentType_Name,TDROLD_DocumentNo,TDROLD_DatePublication,THROLD_Reason,
