...
$ActionContent ="
	...
	<tr>
		...
		<td width='1%'>:</td>
		<td width='60%'>
			<select name='optTransactionID' id='optTransactionID' class='filter'>
				<option value='0'>--- Pilih Jenis Transaksi ---</option>
				<option value='1' ".($_GET['optTransactionID'] == '1' ? 'selected' : '').">Registrasi</option>
...
$ActionContent .="
				<option value='$field[DocumentGroup_ID]' selected>$field[DocumentGroup_Name]</option>"; // Arief F - 17092018
...
$offset = ($noPage - 1) * $dataPerPage;
	...
	if ($_GET[optTHROLD_DocumentGroupID] == '1' or $_GET[optTHROLD_DocumentGroupID] == '2'){
		...
		echo $_GET['optFilterHeader']." ".$_GET['optTransactionID'];
	...
	if ($_GET['optTHROLD_DocumentGroupID'] == '4'){
		if ($_GET['optTransactionID']==1) {
			$query = "SELECT DISTINCT throaod.THROAOD_ID AS ID, throaod.THROAOD_RegistrationCode,
									  throaod.THROAOD_RegistrationDate, u.User_FullName, c.Company_Name,
									  drs.DRS_Description
			...
			$query = "
				SELECT thloaod.THLOAOD_ID AS ID, thloaod.THLOAOD_LoanCode, thloaod.THLOAOD_LoanDate,
					   u.User_FullName, c.Company_Name, lc.LoanCategory_Name,drs.DRS_Description
		...
		if ($_GET['optFilterHeader']==5) {
			if ($_GET['optTransactionID']==1) {
				$query .="AND throaod.THROAOD_RegStatus='$_GET[optFilterDetail]' ";
			}
			elseif ($_GET['optTransactionID']==2) {
	...
	if ($_GET['optTHROLD_DocumentGroupID'] == '5'){
		...
		if ($_GET[optFilterHeader]==4) {
			if ($_GET[optTransactionID]==1) {
				$query .="AND throold.THROOLD_RegStatus='$_GET[optFilterDetail]' ";
			}
			elseif ($_GET[optTransactionID]==2) {
				$query .="AND thlolad.THLOLAD_Status='$_GET[optFilterDetail]' ";
			}
