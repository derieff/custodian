...
<div id='header'>
	...
	$query = "SELECT  thloonld.THLOONLD_ID,
			  ...
			  WHERE thloonld.THLOONLD_LoanCode='$id'
			  AND thloonld.THLOONLD_Delete_Time IS NULL";
	$sql = mysql_query($query);
	$arr = mysql_fetch_array($sql);

	$fregdate=date("j M Y", strtotime($arr['THLOONLD_LoanDate']));
	...
	<table width='100%'>
		<tr>
			...
			<td width='25%'>
				<?PHP echo"$arr[THLOONLD_LoanCode]"; ?>
			</td>
			<td width='15%'>
			...
			<td rowspan='4' align='center'>
				<img src="<?PHP echo "barcode.php?text=$arr[THLOONLD_LoanCode]";?>" /><br />
				<?PHP echo"$arr[THLOONLD_LoanCode]"; ?>
		   </td>
		</tr>
		<tr>
...
<?PHP
	$queryd="SELECT donl.DONL_DocCode, c.Company_Name,
			 ...
			 LEFT JOIN M_DocumentsOtherNonLegal donl
				ON tdloonld.TDLOONLD_DocCode=donl.DONL_DocCode
    ...
    <tr $style>
    	...
    	<td class='center'>
  			$arrd[Department_Name]
        </td>
    	<td class='center'>
  			$arrd[TDLOONLD_Information]
        </td>
    </tr>");
	$jumdata ++;
	}
...
<tfoot>
	<tr>
		<td colspan='18' style='font-size:8px;font-weight:bolder;' align='right'><i>PRINTED BY CUSTODIAN SYSTEM VER <?PHP echo $_SESSION['version']?> ON <?PHP echo date("d/m/Y H:i:s")?></i></td>
	</tr>
</tfoot>
</table>
...
<tr>
	<td><?PHP echo"$arr[THLOONLD_Information]";?></td>
</tr>
</table>
