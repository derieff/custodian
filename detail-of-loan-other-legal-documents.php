...
if(isset($_POST[approval])) {
	...
	$A_GroupDocID = $_POST['txtDOL_GroupDocID'];
			...
			$nnum=$maxnum+1;
				...
				for($i=0;$i<$jumlah;$i++){
					...
					$query1 = "UPDATE M_DocumentsOtherLegal
								SET DOL_Status ='$docStatus',DOL_Update_Time=sysdate(),DOL_Update_UserID='$_SESSION[User_ID]'
					...
					$sql1 = "UPDATE TD_LoanOfOtherLegalDocuments
								SET TDLOOLD_Code ='$CT_Code',TDLOOLD_Update_Time=sysdate(),
									TDLOOLD_Update_UserID='$_SESSION[User_ID]'
		...
		if (($sql = mysql_query($query)) && ($sql1 = mysql_query($query1))) {
			...
			// Mengubah Status Dokumen menjadi "TERSEDIA"
				...
				for($i=0;$i<$jumlah;$i++){
					...
					$query1 = "UPDATE M_DocumentsOtherLegal
								SET DOL_Status ='1', DOL_Update_Time=sysdate(),DOL_Update_UserID='$_SESSION[User_ID]'
								WHERE DOL_DocCode='$txtDOL_DocCode[$i]'";
					$mysqli->query($query1);
				}
