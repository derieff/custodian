<?PHP session_start(); ?>
<title>Custodian System | Peran</title>
<head>
...
function validateInput(elem) {
	...
	var txtRole_Name = document.getElementById('txtRole_Name').value;
		
		if (txtRole_Name.replace(" ", "") == "") {
...
$ActionContent ="
	...
	<tr>
		...
		<td width='70%'><input name='txtRole_Name' id='txtRole_Name' type='text' /></td>
	</tr>
	<th colspan=3>
...
";
	...
	$query = "SELECT * 
				...
				AND Role_Delete_Time is NULL ORDER BY Role_ID ";
...
$ActionContent ="
	...
	<tr>
		...
		<td width='70%'>
			<input name='txtRole_ID' type='text' value='$field[Role_ID]' readonly='true' class='readonly'/>
		</td>
	</tr>
	<tr>
		<td>Nama Peran</td>
		<td><input name='txtRole_Name' id='txtRole_Name' type='text' value='$field[Role_Name]'/></td>
	</tr>
	<th colspan=3>
...
$ActionContent ="
	...
	<tr>
		<td>Nama Peran</td>
		<td><input name='txtRole_Name' type='text' value='$field[Role_Name]' readonly='true' class='readonly'/></td>
	</tr>
	<th colspan=3>
...
$MainContent .="
	<tr>
		<td class='center'>$field[Role_ID]</td>
...
elseif(isset($_POST[add])) {
	$sql= "INSERT INTO M_Role 
				VALUES (NULL,'$_POST[txtRole_Name]','$_SESSION[User_ID]', sysdate(),'$_SESSION[User_ID]', 
						sysdate(),NULL,NULL)";
	if($mysqli->query($sql)) {
		echo "<meta http-equiv='refresh' content='0; url=role.php'>";
	}
...
elseif(isset($_POST[edit])) {
	$sql= "UPDATE M_Role 
				SET Role_Name='$_POST[txtRole_Name]', Role_Update_UserID='$_SESSION[User_ID]', Role_Update_Time=sysdate() 
				WHERE Role_ID='$_POST[txtRole_ID]'";
	if($mysqli->query($sql)) {
		echo "<meta http-equiv='refresh' content='0; url=role.php'>";
	}
	else {
...
elseif(isset($_POST[delete])) {
	$sql= "UPDATE M_Role 
				SET Role_Delete_UserID='$_SESSION[User_ID]', Role_Delete_Time=sysdate() 
				WHERE Role_ID='$_POST[txtRole_ID]'";
	if($mysqli->query($sql)) {
		echo "<meta http-equiv='refresh' content='0; url=role.php'>";
	}
