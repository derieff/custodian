...
function validateInput(elem) {
	...
	if (typeof  document.DL_detail.optTHROAOD_Status != 'undifined') {
	...
	if (typeof  document.DL_detail.optDAO_Employee_NIK != 'undifined') {
		for (i = 1; i < jRow; i++){
			...
			if (txtDAO_STNK_StartDate.replace(" ", "") == "")  {
				alert("Tanggal Mulai Berlaku STNK pada baris ke-" + i + " Belum Terisi!");
				return false
			}
			if (txtDAO_STNK_StartDate.replace(" ", "") != "")  {
				if (checkdate(txtDAO_STNK_StartDate,i) == false) {
					return false
				}
			}
			if (txtDAO_STNK_ExpiredDate.replace(" ", "") != "")  {
				if(txtDAO_STNK_ExpiredDate!="-"){
					if (checkdate(txtDAO_STNK_ExpiredDate,i) == false) {
						return false
					}
				}
				else {
			...
			if (txtDAO_Pajak_StartDate.replace(" ", "") == "")  {
				alert("Tanggal Mulai Berlaku STNK pada baris ke-" + i + " Belum Terisi!");
				return false
			}
			if (txtDAO_Pajak_StartDate.replace(" ", "") != "")  {
				if (checkdate(txtDAO_Pajak_StartDate,i) == false) {
					return false
				}
			}
			if (txtDAO_Pajak_ExpiredDate.replace(" ", "") != "")  {
				if(txtDAO_Pajak_ExpiredDate!="-"){
					if (checkdate(txtDAO_Pajak_ExpiredDate,i) == false) {
						return false
					}
				}
				else {
...
if((($arr[THROAOD_Status]=="accept")||($arr[THROAOD_Status]=="waiting")) && (($custodian==1) || ($regUser==$_SESSION['User_ID']))){
	$MainContent .="
	<tr>
...
if((($act=='edit') && (($custodian==1)||($admin=="1"))) || (($act=='approve')&&($approver=="1")&&($custodian==1)))
	...
	$MainContent .="<td colspan='2'>$arr[THROAOD_Information]</td>";
$MainContent .="</tr>";
...
$query = "SELECT TDROAOD_ID, TDROAOD_Employee_NIK, TDROAOD_MK_ID, TDROAOD_Type, TDROAOD_Jenis,
...
while ($arr = mysql_fetch_array($sql)) {
	if((($act=='edit') && (($custodian==1)||($admin=="1"))) || (($act=='approve')&&($approver=="1")&&($custodian==1))) {
		...
		$stnk_sdate=date("m/d/Y", strtotime($arr['TDROAOD_STNK_StartDate']));
		$stnk_exdate=(($arr['TDROAOD_STNK_ExpiredDate']=="0000-00-00 00:00:00")||($arr['TDROAOD_STNK_ExpiredDate']=="1970-01-01 01:00:00"))?"-":date("m/d/Y", strtotime($arr['TDROAOD_STNK_ExpiredDate']));
		$pajak_sdate=date("m/d/Y", strtotime($arr['TDROAOD_Pajak_StartDate']));
		$pajak_exdate=(($arr['TDROAOD_Pajak_ExpiredDate']=="0000-00-00 00:00:00")||($arr['TDROAOD_Pajak_ExpiredDate']=="1970-01-01 01:00:00"))?"-":date("m/d/Y", strtotime($arr['TDROAOD_Pajak_ExpiredDate']));
		...
		$stnk_exdate=(($arr['TDROAOD_STNK_ExpiredDate']=="0000-00-00 00:00:00")||($arr['TDROAOD_STNK_ExpiredDate']=="1970-01-01 01:00:00"))?"-":date("j M Y", strtotime($arr['TDROAOD_STNK_ExpiredDate']));
		...
		<tr>
			...
			<td class='center'><input name='optDAO_Employee_NIK$no' type='hidden' value='$arr[TDROAOD_Employee_NIK]'>$nama_pemilik</td>
...
if(isset($_POST[cancel])) {
	...
	for($i=1;$i<$count;$i++){
		...
		// $txtDAO_Lokasi_PT='asdasdas2asdasgj';
		// echo "<script>alert('".$txtDAO_Lokasi_PT."');</script>";
		// break;
		// die;
		...
		$query = "UPDATE TD_RegistrationOfAssetOwnershipDocument
				  SET TDROAOD_Employee_NIK='$optDAO_Employee_NIK',
				      ...
				      TDROAOD_Update_UserID='$_SESSION[User_ID]'
		...
		if ($A_Status=='3') {
			...
			if ($step <> $jStep) {
				...
				if ($sql = mysql_query($query)){
					mail_registration_doc($A_TransactionCode);
					if($step=='1'){
						mail_notif_registration_doc($A_TransactionCode, $_POST['txtDAO_RegUserID'], 3 );
