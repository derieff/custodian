		...
		if ($AppDate==NULL) {
			...
			if ($A_Status=='3') {
				...
				if ($step <> $jStep) {
					...
					$qComp = "SELECT Company_Area FROM M_Company WHERE Company_ID = '{$h_arr['THROONLD_CompanyID']}'";
					...
					if ($sql = mysql_query($query)) {
						...
						mail_registration_doc($A_TransactionCode);
						if($step=='1'){
							mail_notif_registration_doc($A_TransactionCode, $h_arr['THROONLD_UserID'], 3 );
					...
					$query="SELECT *
							...
							WHERE DL_Status='1'
							AND DL_CompanyID='$h_arr[THROONLD_CompanyID]'
					...
					if((!$avLoc)||($avLoc<$jumlahRow)){
						...
						$query = "SELECT *
									FROM M_Company
									WHERE Company_ID='$h_arr[THROONLD_CompanyID]'";
						$sql = mysql_query($query);
						...
						$query = "SELECT *
									...
									WHERE DocumentGroup_ID ='$h_arr[THROONLD_DocumentGroupID]'";
						$sql = mysql_query($query);
						$field = mysql_fetch_array($sql);
						...
						while($d_arr=mysql_fetch_array($d_sql)){
							...
							$sql3= "INSERT INTO M_DocumentsOtherNonLegal
									VALUES (NULL,
											'$CD_Code',
											'$h_arr[THROONLD_UserID]',
											'$h_arr[THROONLD_RegistrationDate]',
											'$h_arr[THROONLD_CompanyID]',
											'$h_arr[THROONLD_DocumentGroupID]',
											'$d_arr[TDROONLD_PT_ID]',
											'$d_arr[TDROONLD_NoDokumen]',
											'$d_arr[TDROONLD_NamaDokumen]',
											'$d_arr[TDROONLD_TahunDokumen]',
											'$d_arr[TDROONLD_Dept_Code]',
