...
if(isset($_GET["act"])) {
	...
	if($act=='add') {
		...
		<tr>
			<td width='30'>Nama</td>
			<td width='70%'>
				<input name='txtTHLOOLD_UserID' type='hidden' value='$_SESSION[User_ID]'/>
				$field[FullName]
			</td>
		</tr>
		<tr>
			...
			if( !empty($field['Employee_GradeCode']) && !empty($field['Employee_Grade']) ){
				...
				if ($number>0) {
	                ...
	                <tr>
	                    <td>Tipe Dokumen</td>
	                    <td>
	                        <select name='optTHLOOLD_DocumentType' id='optTHLOOLD_DocumentType'>
					...
					$ActionContent .="
							...
							<input id='txtTHLOOLD_SoftcopyReciever' name='txtTHLOOLD_SoftcopyReciever' type='text' />
						</td>
					</tr>
					<tr>
						<td>Perusahaan</td>
	...
	elseif($act=='adddetail')	{
		...
		<tr>
			...
			if( $field['THLOOLD_DocumentType'] == "ORIGINAL" ){
				$ActionContent .="Asli";
			}elseif( $field['THLOOLD_DocumentType'] == "HARDCOPY" or $field['THLOOLD_DocumentType'] == "SOFTCOPY" ){
				$ActionContent .= ucfirst(strtolower($field['THLOOLD_DocumentType']));
			}else{
				if( $field['THLOOLD_LoanCategoryID'] != '3') $ActionContent .= "Asli";
				else $ActionContent .= "";
			}
			$ActionContent .="</td>
		</tr>
		...
		<tr>
			...
			// for($sApp=1;$sApp<10;$sApp++) {
				...
				if( $jabatan != null ){
					...
					if($field['THLOOLD_DocumentType'] == "ORIGINAL" or $field['THLOOLD_DocumentType'] == "SOFTCOPY"){
...
$MainContent .="
	...
	while ($field = mysql_fetch_array($sql)) {
		...
		<tr>
			<td class='center'>
				<a href='detail-of-loan-other-legal-documents.php?id=$field[0]' class='underline'>$field[1]</a>
			</td>
...
elseif(isset($_POST['canceldetail'])) {
	$cd_query="UPDATE M_CodeTransaction ct, TH_LoanOfOtherLegalDocuments thloold
			   SET ct.CT_Delete_UserID='$_SESSION[User_ID]',ct.CT_Delete_Time=sysdate(),
			       ...
			       thloold.THLOOLD_Delete_UserID='$_SESSION[User_ID]',thloold.THLOOLD_Delete_Time=sysdate(),
			       thloold.THLOOLD_Update_UserID='$_SESSION[User_ID]',thloold.THLOOLD_Update_Time=sysdate()
...
$(document).ready(function(){
	$("#txtTHLOOLD_SoftcopyReciever").hide();
	$("#optTHLOOLD_DocumentType").change(function(){
		...
		if($(this).val()=="ORIGINAL"){
			...
			$("#txtTHLOOLD_SoftcopyReciever").show();
		}
		else{
