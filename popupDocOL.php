...
function pickla(symbol,row) {
	if (window.opener && !window.opener.closed) {
    window.opener.document.adddetaildoc.txtTDLOOLD_DocCode<?= $_GET['row'] ?>.value = symbol;
	window.opener.document.adddetaildoc.docCode.value = window.opener.document.adddetaildoc.docCode.value + "\'" + symbol + "\'" +",";
	window.close();
	}
}
...
IF ($txtTHLOLD_DocumentGroupID<>"3"){
	if ($_GET['recentCode']) $filter =" AND dl.DOL_DocCode NOT IN (".substr($_GET[recentCode],0, -1).")";
	...
	$query="SELECT DISTINCT c.Company_Name, dg.DocumentGroup_Name,
			...
			AND dol.DOL_CompanyID='$txtTHLOLD_CompanyID'
...
ELSE{
	$phase=$_GET['pID'];
	if ($_GET[recentCode]) $filter =" AND dol.DOL_Code NOT IN (".substr($_GET[recentCode],0, -1).")";

	$query="SELECT DISTINCT c.Company_Name, dg.DocumentGroup_Name, dol.DOL_RegTime, dol.DOL_Code, dol.DOL_Phase, dol.DOL_Period,
							dol.DOL_DocDate, dol.DOL_Block, dol.DOL_Village, dol.DOL_Owner
			FROM M_DocumentsOtherLegal dol, M_Company c, M_DocumentGroup dg
			WHERE dol.DOL_Status ='1'
			AND dol.DOL_CompanyID='$txtTHLOLD_CompanyID'
			AND dg.DocumentGroup_ID='$txtTHLOLD_DocumentGroupID'
			AND dol.DOL_Phase='$phase'
			AND dol.DOL_Delete_Time IS NULL
	...
	if ($numRow==0) {
		...
		$h_sql=mysql_query($query);
		$h_arr=mysql_fetch_array($h_sql);
		$period=date("d M Y", strtotime($h_arr[DOL_Period]));
		echo "<form name='search' method='post' action='$PHP_SELF'>
			  <div style='text-align:left; padding:10px 5px; margin-bottom :5px; background :#CCC;'>
				<b>Pencarian :</b> <input name='txtSearch' id='txtSearch' type='text' size='25%'/>
			  </div>
			  </form>";
		echo "<div class=title><b>$h_arr[Company_Name] - $h_arr[DocumentGroup_Name]</b><br><i>Tahap $h_arr[DOL_Phase] : $period</i></div>";
		?>
		...
		if($_POST) {
			$search=$_POST['txtSearch'];
			$query =   "SELECT c.Company_Name, dg.DocumentGroup_Name, dol.DOL_RegTime, dol.DOL_Code, dol.DOL_Phase, dol.DOL_Period,
							   dol.DOL_DocDate, dol.DOL_Block, dol.DOL_Village, dol.DOL_Owner
						FROM M_DocumentsOtherLegal dol, M_Company c, M_DocumentGroup dg
						WHERE dol.DOL_Status ='1'
						AND dol.DOL_CompanyID='$txtTHLOLD_CompanyID'
						...
						$filter
						AND (
							dol.DOL_Code LIKE '%$search%'
							OR dol.DOL_DocDate LIKE '%$search%'
							OR dol.DOL_Block LIKE '%$search%'
							OR dol.DOL_Village LIKE '%$search%'
							OR dol.DOL_Owner LIKE '%$search%'
						)
						ORDER BY dol.DOL_RegTime DESC ";
			$limit = " LIMIT $posisi, $batas";
			$sql = mysql_query($query.$limit);
		...
		while ($arr=mysql_fetch_array($sql)){
			$docdate=date("d M Y", strtotime($arr[DOL_DocDate]));
			?>
			<tr>
				<td align='center'><u><a href="javascript:pickla('<?= $arr['DOL_Code'] ?>','<?= $_GET['row'] ?>')"><?= $arr['DOL_Code'] ?></a></u></td>
				<td align='center'><?= $docdate ?></td>
				<td align='center'><?= $arr['DOL_Block'] ?></td>
				<td align='center'><?= $arr['DOL_Village'] ?></td>
				<td align='center'><?= $arr['DOL_Owner'] ?></td>
			</tr>
			<?PHP
		}
	}
