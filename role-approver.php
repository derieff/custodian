	        ...
	        var nextDoc = "<tr>\
				...
				<td class='center'>\
					<select name='dokumen[]'>\
						...
						<option value='14'>Kepemilikan Aset - Asli & Softcopy</option>\
						<option value='15'>Kepemilikan Aset - Hardcopy</option>\
						<option value='16'>Lainnya (Legal) - Semua</option>\
						<option value='17'>Lainnya (Legal) - Asli & Softcopy</option>\
...
$arrayDoc = array(
	...
	'14' => 'Kepemilikan Aset - Asli & Softcopy', //Arief F - 13092018
	'15' => 'Kepemilikan Aset - Hardcopy', //Arief F - 13092018
	'16' => 'Lainnya (Legal) - Semua', //Arief F - 13092018
	'17' => 'Lainnya (Legal) - Asli & Softcopy', //Arief F - 13092018
	...
	if($mysqli->query($sql)) {
		...
		if (isset($_POST['dokumen']) && isset($_POST['step']) && isset($_POST['status']) && !empty($_POST['dokumen']) && !empty($_POST['step']) && !empty($_POST['status'])) {
			...
			foreach ($_POST['dokumen'] as $k=>$v) {
				$sql1 .= "(NULL, '$last_id', '{$_POST['proses'][$k]}', '$v', '{$_POST['step'][$k]}', '{$_POST['status'][$k]}', '$_SESSION[User_ID]', sysdate(), '$_SESSION[User_ID]', sysdate(), NULL, NULL), ";
...
elseif(isset($_POST[edit])) {
	...
	if($mysqli->query($sql)) {
		...
		$sql1 = "INSERT INTO M_Role_ApproverDocStepStatus VALUES ";
		foreach ($_POST['dokumen'] as $k=>$v) {
			$sql1 .= "(NULL, '$_POST[txtRA_ID]', '{$_POST['proses'][$k]}', '$v', '{$_POST['step'][$k]}', '{$_POST['status'][$k]}', '$_SESSION[User_ID]', sysdate(), '$_SESSION[User_ID]', sysdate(), NULL, NULL), ";
		}
...
elseif(isset($_POST[delete])) {
	$sql= "UPDATE M_Role_Approver
				...
				WHERE RA_ID='$_POST[txtRA_ID]'";
	if($mysqli->query($sql)) {
		$sql1 = "UPDATE M_Role_ApproverDocStepStatus SET RADS_Delete_UserID='$_SESSION[User_ID]', RADS_Delete_Time=sysdate() WHERE RADS_RA_ID='$_POST[txtRA_ID]'";
		if ($mysqli->query($sql)) {
			echo "<meta http-equiv='refresh' content='0; url=role-approver.php'>";
		}
		echo "<meta http-equiv='refresh' content='0; url=role-approver.php'>";
