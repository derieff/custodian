...
if(isset($_GET["act"]))
{
	...
	if($act=='add') {
		...
		<tr>
			<td width='30'>Nama</td>
			<td width='70%'>
				<input id='txtTHROAOD_UserID' type='hidden' value='$_SESSION[User_ID]'/>
				$field[FullName]
			</td>
		</tr>
		<tr>
	...
	for ($i=0 ; $i<$sum ; $i++) {
		...
		$sql1= "INSERT INTO TD_ReleaseOfAssetOwnershipDocument
				VALUES (NULL,NULL,'$_POST[txtTDROAOD_THROAOD_ID]', '$TDLOAOD_ID[$i]','$txtTDROAOD_Information[$i]',
						'$TDROAOD_LeadTime',NULL,'$_SESSION[User_ID]', sysdate(),'$_SESSION[User_ID]',
						sysdate(),NULL,NULL)";
		$sql2= "UPDATE TD_LoanOfAssetOwnershipDocument
				SET TDLOAOD_Response='1', TDLOAOD_Update_UserID='$_SESSION[User_ID]',TDLOAOD_Update_Time=sysdate()
