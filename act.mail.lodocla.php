		...
		if ($AppDate==NULL) {
			...
			if ($A_Status=='3') {
				...
				if ($step <> $jStep) {
					...
					// if ($h_arr['THLOLAD_LoanCategoryID'] != '3') { $jenis = '5'; }
							...
							************************************/
							...
							/*if ($i == $jStep) {
								$query = "UPDATE TH_LoanOfLandAcquisitionDocument
									...
									AND THLOLAD_Delete_Time IS NULL";
								if ($sql = mysql_query($query)) {
									mail_notif_loan_doc($A_TransactionCode, $h_arr['THLOLAD_UserID'], 3, 1 );
							...
							if ($yquery['abc'] != '0') {
								...
								$query = "UPDATE M_Approval
											...
											WHERE A_TransactionCode='$A_TransactionCode' AND A_Step='$i'";
								if ($sql = mysql_query($query)) {
									mail_notif_loan_doc($A_TransactionCode, $result['A_ApproverID'], 3, 0);
								}
							}
