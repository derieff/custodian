...
$query =  "	SELECT DISTINCT thloaod.THLOAOD_ID,
						  ...
						  c.Company_ID,
						  lc.LoanCategory_ID,
						  thloaod.THLOAOD_SoftcopyReciever,
...
<tr>
	<td>Nama Peminta</td>
	<td colspan='2'><input name='txtDAO_RegUserID' type='hidden' value='$arr[User_ID]'>$arr[User_FullName]</td>
</tr>
<tr>
...
$MainContent .="<tr>
	...
	<td colspan='2'>
		...
		if($arr['THLOAOD_DocumentType'] != "SOFTCOPY"){
			...
			$MainContent .="<input id='txtTHLOAOD_SoftcopyReciever' name='txtTHLOAOD_SoftcopyReciever' type='hidden' value='THLOAOD_SoftcopyReciever'/>
...
while ($arr = mysql_fetch_array($sql)) {
	...
	$stnk_exdate=(($arr['DAO_STNK_ExpiredDate']=="0000-00-00 00:00:00")||($arr['DAO_STNK_ExpiredDate']=="1970-01-01 01:00:00"))?"-":date("j M Y", strtotime($arr['DAO_STNK_ExpiredDate']));
			...
			$nnum=$maxnum+1;
				...
				for($i=0;$i<$jumlah;$i++){
					...
					$query1 = "UPDATE M_DocumentAssetOwnership
								SET DAO_Status ='$docStatus',DAO_Update_Time=sysdate(),DAO_Update_UserID='$_SESSION[User_ID]'
					...
					$sql1 = "UPDATE TD_LoanOfAssetOwnershipDocument
								SET TDLOAOD_Code ='$CT_Code',TDLOAOD_Update_Time=sysdate(),
									TDLOAOD_Update_UserID='$_SESSION[User_ID]'
		...
		if (($sql = mysql_query($query)) && ($sql1 = mysql_query($query1))) {
			...
			// Mengubah Status Dokumen menjadi "TERSEDIA"
				...
				for($i=0;$i<$jumlah;$i++){
					...
					$query1 = "UPDATE M_DocumentAssetOwnership
								SET DAO_Status ='1', DAO_Update_Time=sysdate(),DAO_Update_UserID='$_SESSION[User_ID]'
								WHERE DAO_DocCode='$txtDAO_DocCode[$i]'";
					$mysqli->query($query1);
				}
