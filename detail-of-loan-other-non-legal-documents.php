...
while ($arr = mysql_fetch_array($sql)) {
	...
	<tr>
		...
		<td class='center'><input name='txtDONL_PT_ID[]' type='hidden' value='$arr[DONL_PT_ID]'>$arr[Company_Name]</td>
				...
				for($i=0;$i<$jumlah;$i++){
					...
					$sql1 = "UPDATE TD_LoanOfOtherNonLegalDocuments
								SET TDLOONLD_Code ='$CT_Code',TDLOONLD_Update_Time=sysdate(),
									TDLOONLD_Update_UserID='$_SESSION[User_ID]'
		...
		if (($sql = mysql_query($query)) && ($sql1 = mysql_query($query1))) {
			...
			// Mengubah Status Dokumen menjadi "TERSEDIA"
				...
				for($i=0;$i<$jumlah;$i++){
					...
					$query1 = "UPDATE M_DocumentsOtherNonLegal
								SET DONL_Status ='1', DONL_Update_Time=sysdate(),DONL_Update_UserID='$_SESSION[User_ID]'
								WHERE DONL_DocCode='$txtDONL_DocCode[$i]'";
					$mysqli->query($query1);
				}
